...
module QueueClassicJobsManager
  ...
  def start_workers
    ...
    pass = uri.password
    db   = uri.path[1..-1]
    puts %x{#{"PGPASSWORD=\"#{pass}\"" if pass} psql -X -c 'drop database if exists "#{db}"' -U #{user} -t template1}
    puts %x{#{"PGPASSWORD=\"#{pass}\"" if pass} psql -X -c 'create database "#{db}"' -U #{user} -t template1}
    QC::Setup.create
